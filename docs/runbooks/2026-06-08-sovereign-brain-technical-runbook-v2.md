...
- `fleet/routing/cuopt_adapter.py`: cuOpt transport (yêu cầu polling kết quả bất đồng bộ)
...
- Yêu cầu mở SSH Tunnel để chạy cuOpt API bất đồng bộ: `ssh -N -L 8001:localhost:8001 <user>@<server_ip>`
...
$env:CUOPT_ENDPOINT="localhost:8001"
...
- Kết nối: Yêu cầu mở SSH Tunnel `ssh -N -L 8001:localhost:8001 <user>@<server_ip>` do API cuOpt NIM dùng polling bất đồng bộ.
- Adapter: `fleet/routing/cuopt_adapter.py`
- Expected format: `localhost:8001` (khi dùng qua SSH Tunnel)
- Failure mode: transport exception tại `solve()` hoặc kẹt polling do cấu hình/lệnh sai.
...
- Flow chuẩn cần tuân thủ là:
  1. smoke runtime
  2. thiết lập cổng nối với server (SSH Tunnel) và kiểm endpoint cuOpt/NIM
