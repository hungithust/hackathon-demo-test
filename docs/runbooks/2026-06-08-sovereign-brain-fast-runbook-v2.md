...
Nếu có cuOpt (kết nối qua SSH Tunnel do API Server chạy NVIDIA cuOpt NIM bất đồng bộ ở cổng 8001):

Mở một Terminal/CMD ở máy Local và chạy lệnh sau (treo cửa sổ để duy trì kết nối):

```powershell
ssh -N -L 8001:localhost:8001 <user>@<server_ip>
```

Sau đó, ở cửa sổ PowerShell chính:
...
- Nếu dùng `ROUTING_ENGINE=cuopt`, phải đảm bảo đang mở SSH Tunnel tới port 8001 và endpoint reachable
